...
// function to return the variables in the expression like (A+C+D)*E ---> A,C,D,E Therfore, we will know the number of variables and corresponding variable to each column in the truth table
...
vector<char> CorrespodingVariables(string Term)
{
...
string formImplicantToTerm(string implicant)
{
    string Term;
    char letter = 'A';
...
void PrintAllPrimeImplicants(vector<Implicant> implicants)
{
    ...
    {
        cout << setw(6) <<formImplicantToTerm(implicants[j].term) << " :: " << implicants[j].term << "   ";
        cout << " (";
...
vector<string> epi(vector <Implicant>& prim, vector<int>& m)
{
    ...
    for (int i = 0; i < minterms.size(); i++)
    {
        ...
        //cout << "first row " << table[0][i + 1] << endl;
    }
        ...
        while (test2)
        {
            ...
            case 5:
                ...
                if (input_type == 1)
                {
                    ...
                    cout << " Here are all the EPIs " << endl;
                    for (int i = 0; i < AllEPI.size(); i++)
                        cout << setw(6) << formImplicantToTerm(AllEPI[i]) <<" :: " << AllEPI[i] <<endl;
                }
                else if (input_type == 2|| input_type==3)
                {
                    ...
                    for (int i = 0; i < AllEPI.size(); i++)
                    {
                        cout <<setw(5) << formImplicantToTerm(AllEPI[i]) << " :: "<<AllEPI[i] <<endl;
                    }
                    cout << endl;
                }
                break;
            case 6:
                if (input_type == 1)
                {
                    ...
                    all = getPrimeImplicants(Minterms);
                    AllEPI = epi(all, allminterms);
                    cout << "The Implicants that are not covered by EPIs : ";
                    for (int i = 0; i < allminterms.size(); i++)
                    {
                        if (i == 0)
                            cout << "(";
                        cout << setw(4) << allminterms[i];
                        if (i != allminterms.size() - 1)
                            cout << " ";
                        else cout << ")";
                    }
                    cout << endl;
